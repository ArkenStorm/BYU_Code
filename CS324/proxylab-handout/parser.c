...
int parse_request(const char *request, char *method, char *hostname, char *port, char *uri, http_header *headers) {
	...
	strcpy(uri,strtok_r(NULL," ",&req_ptr));  // obtains the URL - req_ptr helps the tokenizer continue from the last call
	
	char *url_ptr;
	strcpy(uri,strtok_r(uri,"//",&url_ptr));  // move the ptr past the "http://"
