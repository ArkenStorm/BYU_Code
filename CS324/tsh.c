...
{
	char* argv[100];
	int job_state = parseline(cmdline, argv) ? BG : FG;

	int cmds[100] = {0};
	int stdin_redir[100] = {0};
	int stdout_redir[100] = {0};
	...
	for (int i = 0; i < num_cmds; i++) {
		int pipefd[2] = {0, 1};
		if (cmds[i + 1] > 0) { // if not on the last command
			if (pipe(pipefd) < 0) {
		...
		if (stdin_redir[i] != -1) {
			FILE* in_file = fopen(argv[stdin_redir[i]], "r");
			dup2(fileno(in_file), 0);
		}

		if (stdout_redir[i] != -1) {
			FILE* out_file = fopen(argv[stdout_redir[i]], "w");
			pipefd[1] = fileno(out_file);
		}
		...
		if (child_pid == 0) {
			...
			addjob(jobs, child_pid, getpgid(child_pid), job_state, cmdline);
			sigprocmask(SIG_UNBLOCK, &mask, &prev_mask);
		}
	}
	...
	int fg_job_pid = fgpid(jobs);
	if (job_state == FG) {
		waitfg(fg_job_pid);
	}
	else {
		printf("[%d] (%d) %s\n", getjobpid(jobs, child_pid)->jid, fg_job_pid, cmdline);
	}
...
void do_bgfg(char **argv) 
{
	...
	//atoi may not be what I need?
	if (argv[1][0] == '%') {
		int check_jid = atoi(argv[1]+1);
		job = getjobjid(jobs, check_jid);
		if (job == NULL) {
		...
		int check_pid = atoi(argv[1]);
		job = getjobpid(jobs, check_pid);
		if (job == NULL) {
...
void sigint_handler(int sig) 
{
	int fg_job_pid = fgpid(jobs);
	if (fg_job_pid != 0) {
		struct job_t* fg_job = getjobpid(jobs, fg_job_pid);
