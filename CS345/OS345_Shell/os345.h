...
typedef Tuple* PQueue; // Priority queue
...
typedef struct node { // deltaclock node
	struct Node* next;
	int tickTime; // relative time before the semaphore gets signaled
	Semaphore* sem; // whatever semaphore that needs to get signaled when tickTime reaches zero
} Node;

typedef Node* DeltaClock;
