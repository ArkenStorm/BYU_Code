...
void mySigContHandler(void) { // task mySIGCONT handler
	return;
}
...
void mySigTermHandler(void) { // task mySIGTERM handler
	killTask(curTask);
	return;
}

void mySigTstpHandler(void) { // task mySIGTSTP handler
...
int P1_main(int argc, char* argv[]) {
	...
	while (1) {
		...
		// init arguments
		newArgc = 1;
		int numArgs = 0; // update this eventually
		size_t size = 0;
		...
		while (end != NULL) {
			int quotation = FALSE;
			background = FALSE;
			while (isspace(*start)) // skip leading whitespace
				start++;
			if (*start == '"') {
				quotation = TRUE;
				start++;
				...
				if (end == NULL) {
					quotation = FALSE; // treat it like a normal arg
				}
				else {
					size = end - start;
				}
			}
			if (*start == '&') // also check if start++ is anything?
				background = TRUE;

			if (!quotation) {
				end = strchr(start, ' ');
				size = (end == NULL) ? strlen(start) : (end - start);
				/*if (end != NULL) // checking for & in the middle of a string?
					background = FALSE;*/
			}
		...
		// setting the unused args to 0, i.e. with 5 arguments, set everything in myargv starting with index 5 to 0
		for (i = numArgs; i < MAX_ARGS; i++)
			myArgv[i] = 0;
		newArgc = numArgs;
		newArgv = malloc(sizeof(char*) * (newArgc + 1));
		for (i = 0; i < newArgc; i++) {
			newArgv[i] = malloc(sizeof(char) * (strlen(myArgv[i]) + 1));
		...
		for (found = i = 0; i < NUM_COMMANDS; i++) {
			if (!strcmp(newArgv[0], commands[i]->command) ||
				...
				int retValue = background ? createTask(newArgv[0], commands[i]->func, 1, newArgc, newArgv) : 
						(*commands[i]->func)(newArgc, newArgv);
				if (retValue && !background)
					printf("\nCommand Error %d\n", retValue);
				found = TRUE;
				break;
			}
...
int P1_add(int argc, char* argv[]) {
	...
	for (int i = 1; i < argc; i++) {
		if (strchr(argv[i], '.')) { // float
			total += strtof(argv[i], NULL);
		}
		else if (argv[i][0] == '%') { // binary
			total += (float)strtol(++argv[i], NULL, 2);
		}
		else { // hex, octal, normal
			total += strtol(argv[i], NULL, 0);
		}
	}
	printf("\n%f\n", total);
	return 0;
...
Command** P1_init() {
	...
	commands[i++] = newCommand("add", "ad", P1_add, "Add any amount of numbers in hex or decimal");
	commands[i++] = newCommand("args", "arg", P1_args, "List program arguments");
	...
	commands[i++] = newCommand("testDeltaClock", "tdc", P3_tdc, "Test the deltaclock");
	...
	commands[i++] = newCommand("test4", "t4", P4_test4, "Test vmaccess");
